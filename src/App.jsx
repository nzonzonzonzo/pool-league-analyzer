    ...
    switch (action.type) {
      ...
      case ACTION_TYPES.LOAD_DATA_SUCCESS:
        debugLog('StateChanges', '[REDUCER] Data loaded successfully', {
          matchesCount: action.payload.filteredMatches?.length,
...
function calculateWinRatesByHandicap(playerName, filteredMatches) {
  ...
  // Input validation
  if (!playerName || !filteredMatches || !Array.isArray(filteredMatches)) {
    debugLog('Errors', 'Invalid inputs for calculateWinRatesByHandicap', { 
      playerName, 
      filteredMatchesValid: Boolean(filteredMatches && Array.isArray(filteredMatches)) 
    });
...
function getHeadToHeadRecord(player1, player2, filteredMatches) {
  ...
  // Input validation
  if (!player1 || !player2 || !filteredMatches || !Array.isArray(filteredMatches)) {
  ...
  const directMatches = filteredMatches.filter(match => {
    try {
      const homePlayerNorm = normalizePlayerName(match.homePlayer);
      const awayPlayerNorm = normalizePlayerName(match.awayPlayer);
...
function calculateWinProbability(player1, player2, teamStats, filteredMatches) {
  ...
  const h2h = getHeadToHeadRecord(player1, player2, filteredMatches);
  let h2hAdjustment = 0;
  ...
  const p1VsHandicap = calculateWinRatesByHandicap(player1, filteredMatches);
  const p2VsHandicap = calculateWinRatesByHandicap(player2, filteredMatches);
...
function findBestResponsePlayer(gameNumber, opponentPlayer, availableHomePlayers, availableAwayPlayers, selectedPlayers, teamStats, filteredMatches) {
  ...
  if (!teamStats || !filteredMatches) {
    debugLog('Errors', 'Missing teamStats or filteredMatches in findBestResponsePlayer');
    return null;
  }
...
function findOptimalBlindPlayer(availableHomePlayers, availableAwayPlayers, teamStats, filteredMatches) {
  ...
  if (!teamStats || !filteredMatches) {
    debugLog('Errors', 'Missing teamStats or filteredMatches in findOptimalBlindPlayer');
    return null;
  }
